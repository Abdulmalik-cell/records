...
if (isset($_GET['file_no'])) {
    ...
    $select = "SELECT * FROM records WHERE file_no = '$file_no'";
...
if (isset($_POST['submit'])) {
    ...
    $update = "UPDATE records SET box_no='$box', file_title='$title', date_from='$date_f', date_to='$date_t' WHERE file_no='$file_n'";
    ...
    if ($run) {
        ...
        $select = "SELECT * FROM records WHERE file_no = '$file_n'";
        $result = mysqli_query($link, $select);
        if (mysqli_num_rows($result) > 0) {
            $rows = mysqli_fetch_assoc($result);
        }
